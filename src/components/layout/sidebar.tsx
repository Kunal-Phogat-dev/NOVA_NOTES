...
function SidebarContent({ mobileOpen, setMobileOpen }: { mobileOpen?: boolean; setMobileOpen?: (val: boolean) => void }) {
  ...
  return (
    <>
      ...
      <aside className={`
        fixed inset-y-0 left-0 z-50 w-64 border-r border-border bg-secondary/95 backdrop-blur-md h-screen flex flex-col transition-transform duration-300 ease-in-out
        md:relative md:translate-x-0 md:bg-secondary/20
      ...
      `}>
        ...
        <div className="p-4 border-b border-border flex items-center justify-between">
          ...
          {mobileOpen && (
            <Button variant="ghost" size="icon" onClick={() => setMobileOpen?.(false)} className="md:hidden h-8 w-8 text-muted-foreground">
        ...
        <div className="p-4 space-y-2">
          ...
          <Button variant="outline" className="w-full justify-start text-sm h-9 text-muted-foreground border-border bg-background" asChild>
            <Link href="/notes" onClick={() => setMobileOpen?.(false)}>
              <Search className="mr-2 w-4 h-4" /> Search <span className="ml-auto text-xs opacity-50 hidden md:inline">⌘K</span>
            </Link>
          </Button>
        </div>
